...
public class Hello {
    ...
    // 获取http请求，参数分别为端口号、路径、参数、值
    public String getHttp(int port, String path, String param, String value) {
        try {
            HttpUrl url = new HttpUrl.Builder().scheme("http").host("localhost").port(port).addPathSegment(path)
                    .addQueryParameter(param, value).build();
            Request.Builder requestBuilder = new Request.Builder().url(url);
            //SPAN_KIND:基于RPC的调用角色，"client" 或 "server". 基于消息的调用角色，"producer" 或 "consumer"
    ...
    public static void main(String[] args) {
        ...
        try (JaegerTracer tracer = Tracing.init("hello-opentracing")) {//设置服务名字
